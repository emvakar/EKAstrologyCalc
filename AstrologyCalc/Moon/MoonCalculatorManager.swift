...
//            guard let moonSet = model.moonSet, Calendar(identifier: .gregorian).isDate(moonSet, inSameDayAs: date) else {
//                return
//            }
//
...
//            if let moonSet = $0.moonSet, !Calendar(identifier: .gregorian).isDate(moonSet, inSameDayAs: date) {
//                $0.moonSet = nil
//            }
//
//            // ищем восходы, которые не принадлежат текущему дню и удаляем их
//            if let moonRise = $0.moonRise, !Calendar(identifier: .gregorian).isDate(moonRise, inSameDayAs: date) {
//                return true
//            }
//
//            return false
...
//    }
    ...
    public func getMoons(city: DBCityModel) -> [DBMoonDayModel] {

        let models = city.moonDays
        return models
    }
...
//        var rp: Double = 0.0
//
//        let year: Double = Double(Calendar(identifier: .gregorian).component(.year, from: date))
//        let month: Double = Double(Calendar(identifier: .gregorian).component(.month, from: date))
//        let day: Double = Double(Calendar(identifier: .gregorian).component(.day, from: date))
//
...
//    }
    ...
    private func endOfDate(_ date: Date) -> Date? {
        ...
        components.day = 1
        components.second = -1
        let endDate = Calendar(identifier: .gregorian).date(byAdding: components, to: self.startOfDate(date))
        return endDate
    }
